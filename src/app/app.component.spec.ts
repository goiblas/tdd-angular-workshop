...
describe('AppComponent', () => {
  ...
  xit('1. should do something', async(() => {
    expect(component).toBeTruthy();
  }));

  xit(`2. should have 'TDD Workshop' as title`, async(() => {
    expect(component.title).toEqual('TDD Workshop');
  }));

  xit('3. should render the navigation bar', async(() => {
    const nav = element.querySelector('nav');
    expect(nav).toBeTruthy();
    expect(nav.classList).toContain('navbar');
  }));

  xit('4. should render title in an anchor tag', async(() => {
