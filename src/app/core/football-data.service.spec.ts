...
describe('FootballDataService', () => {
  ...
  describe('Method: getCompetitions', () => {
    ...
    xit('11. should retrieve an array of competitions', () => {
      const testData = [{
        'id': 424,
      ...
      expect(req.request.headers.get('X-Auth-Token')).toBeTruthy();
      req.flush(testData);
    });
  ...
  describe('Method: getCompetitionStandings', () => {
    ...
    xit('13. should retrieve competition standings', () => {
      const testData = {
      ...
      service.getCompetitionStandings(455).subscribe(data => expect(data).toEqual(testData.standing));
